...
public class Song {
    ...
    private URL audioFile; // wav file containing song audio
    ...
    public Song(String t) { // pass in title of song
        ...
        audioFile = Song.class.getResource(t + "/" + t + ".wav");
        infoFile = Files.getInputStream(t + "/" + t + ".txt");
        notesFile = Files.getInputStream(t + "/" + t + "_notes.txt");
        keysFile = Files.getInputStream(t + "/" + t + "_keys.txt");
    ...
    public int getPercentage(int p) { // pass in points earned value
        return (100 * p / totalNotes); // calculate a percentage using points earned parameter and total points possible, cast to int to truncate
    }
