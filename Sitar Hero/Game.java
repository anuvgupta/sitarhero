...
public class Game extends JPanel implements KeyListener { //declare class game as a type of JPanel that is also a KeyListener
    ...
    public Game() { //constructor for Game
        ...
        for (int i = 0; i < names.length; i++) gifs[i] = new File("./files/sitar/" + names[i] + ".gif"); //initialize files for gifs
    ...
    public void setComponents() {
        ...
        Font valueFont = new Font("Helvetica", Font.PLAIN, 70);
        ...
        keysL.setFont(new Font("Helvetica", Font.BOLD, 17));
        ...
        Font finishFont1 = new Font("Helvetica", Font.PLAIN, 70);
        ...
        try { //try block for buffering images without crashing
            img = ImageIO.read(new File("./files/sitar/sitar.png")); //use static read() method from ImageIO class to load an image into img
            ...
            gameTimer.scheduleAtFixedRate(new TimerTask() { //schedule a new custom TimerTask for the timer; use scheduleAtFixedRate to reduce lag from swing and audio functions
                    public void run() { //declare a run method for override
                        ...
                        for (int i = 0; i < 6; i++) { //loop through list of lists of Notes
                            ...
                            if (notes.get(i).size() > 0) empty = false; //if even a single string has a note on it, empty will be set to false
                        }
    ...
    public void keyPressed(KeyEvent e) { //when a key is pressed
        if (gameState == 1) { //if the game is running
            ...
            if (gameState == 4) { //if the game is in a win state
                ...
                else level++;; //but if the player is jsut at the end  of a regular level, increment level before reconstructing
            }
