...
public class Song { 
    ...
    private InputStream infoFile; // text file with song info (full name, bpm, npb)
    private InputStream keysFile; // text file containing song keys in order
    private InputStream notesFile; // text file containing song notes in order
    private InputStream audioFile; // wav file containing song audio
    ...
    public Song (String t) { // pass in title of song
        ...
        String path1 = "files/" + t + "/" + t + ".wav";
        String path2 = "files/" + t + "/" + t + ".txt";
        String path3 = "files/" + t + "/" + t + "_notes.txt";
        String path4 = "files/" + t + "/" + t + "_keys.txt";
        audioFile = Song.class.getClassLoader().getResourceAsStream(path1);
        infoFile = Song.class.getClassLoader().getResourceAsStream(path2);
        notesFile = Song.class.getClassLoader().getResourceAsStream(path3);
        keysFile = Song.class.getClassLoader().getResourceAsStream(path4);
        ...
        try { // try catch block for catching any thrown exceptions in the song loading
            ...
            AudioInputStream audioStream = AudioSystem.getAudioInputStream(new BufferedInputStream(audioFile)); // load audio file data into audio input stream object 
